...
**Broader Replacement 사용:**
구체적 정보를 찾을 수 없으면 더 넓은 범위의 검증된 정보로 대체하세요.
예시:
- "이 와인이 파커 100점" (검증 실패) → "파커가 이 지역을 극찬" (검증 성공)
- "1872년 설립" (검증 실패) → "19세기부터 이어진 생산자" (검증 성공)
- "2015 빈티지 파커 98점" (검증 실패) → "2015는 이 지역의 위대한 빈티지로 평가받습니다" (검증 성공)
